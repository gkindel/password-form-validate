...
import {validate} from "./PasswordValidation.ts";
import {Register} from "./useFormValidator.ts";
...
export const PasswordInput:React.FC<PasswordInputProps> = (
    ...
    useEffect(
        () =>  register && register(name, () => validate(primary, secondary)),
        [name, primary, register, secondary]
    );

    const _handlePrimaryChange = useCallback((e:React.ChangeEvent):void => {
        setPrimary((e.currentTarget as HTMLInputElement).value);
    },[setPrimary]);

    const _handleSecondaryChange = useCallback((e:React.ChangeEvent):void => {
    ...
    return (
        <>
            <StyledLabel>
                ...
                <input type={"password"} data-testid={'input_password'} name={name} value={primary} onChange={_handlePrimaryChange}/>
            </StyledLabel>
            <StyledLabel>
